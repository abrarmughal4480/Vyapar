...
export const createDeliveryChallan = async (req, res) => {
  try {
    ...
    if (data.items && Array.isArray(data.items)) {
      for (const challanItem of data.items) {
        const dbItem = await Item.findOne({ userId: userId.toString(), name: challanItem.item });
        if (dbItem) {
          const requestedQty = Number(challanItem.qty) || 0;
          ...
          stockCheckResults.push({
            item: challanItem.item,
            available: false,
            requestedQty: Number(challanItem.qty) || 0,
            availableStock: 0,
