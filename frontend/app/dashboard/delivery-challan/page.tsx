...
export default function DeliveryChallanPage() {
  ...
  const simulateLocalApiCall = useCallback(async (endpoint: string, options: RequestInit = {}): Promise<any> => {
    ...
    if (endpoint.includes('/delivery-challan')) {
      if (options.method === 'POST') {
        ...
        const existingChallans = JSON.parse(localStorage.getItem('vyparr_delivery_challans_biz_12345') || '[]');
        existingChallans.unshift(newChallan);
        localStorage.setItem('vyparr_delivery_challans_biz_12345', JSON.stringify(existingChallans));
      ...
      const savedChallans = JSON.parse(localStorage.getItem('vyparr_delivery_challans_biz_12345') || '[]');
  ...
  const fetchDeliveryChallans = useCallback(async () => {
    try {
      const result = await makeApiCall(`/delivery-challan/biz_12345`);
