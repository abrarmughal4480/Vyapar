...
export default function PartiesPage() {
  ...
  useEffect(() => {
    ...
    // Removed: if (searchParams?.get('addParty') === '1') {
  ...
  return (
    <>
      <Suspense fallback={null}>
        <PartiesSearchParamsClient onParams={(params) => {
      ...
      <div className="p-6 bg-gray-50 min-h-screen">
        ...
        {/* Party Categories - pill, color, selected */}
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow p-4 md:p-6 mb-6 border border-gray-100 z-[1]">
          ...
          <div className="flex flex-wrap gap-3 md:gap-4">
            {categories.map((category) => {
              // Unique icon and color for each category (PKR/business relevant)
              let bg = 'bg-gray-100', text = 'text-gray-800', iconBg = 'bg-gray-200', icon = '';
              switch (category.name) {
                case 'Customers':
                  bg = 'bg-blue-50'; text = 'text-blue-800'; iconBg = 'bg-blue-100'; icon = '🧑‍💼'; // Business person
                  break;
                case 'Suppliers':
                  bg = 'bg-yellow-50'; text = 'text-yellow-800'; iconBg = 'bg-yellow-100'; icon = '🏢'; // Office building
                  break;
                case 'Debtors':
                  bg = 'bg-green-50'; text = 'text-green-800'; iconBg = 'bg-green-100'; icon = '💸'; // Money with wings
                  break;
                case 'Creditors':
                  bg = 'bg-red-50'; text = 'text-red-800'; iconBg = 'bg-red-100'; icon = '💰'; // Money bag
                  break;
                default:
                  icon = '👤';
              }
              return (
                ...
                >
                  <span className={`w-9 h-9 flex items-center justify-center rounded-full text-2xl font-bold ${iconBg}`}>{icon}</span>
                  <div className="flex flex-col">
                    <span className="font-semibold text-base leading-tight">{category.name}</span>
                    <span className="text-xs text-gray-500">{category.count} parties</span>
                  </div>
                  <span className={`ml-4 font-bold ${getBalanceColor(category.totalBalance)} text-base`}>PKR {Math.abs(category.totalBalance).toLocaleString()}</span>
                </div>
              )
            })}
          </div>
              ...
              <div className="flex flex-col space-y-3 sm:flex-row sm:space-y-0 sm:space-x-4">
                ...
                <div ref={dropdownRef} className="relative w-full sm:w-60">
                  <button
                    ...
                    onClick={() => setShowCategoryDropdown((v: boolean) => !v)}
                  ...
                  {showCategoryDropdown && typeof window !== 'undefined' && ReactDOM.createPortal(
                    ...
                    >
                      ...
                      <li
                        className={`px-4 py-2 cursor-pointer rounded-lg transition-all hover:bg-blue-50 ${selectedCategory === 'Customer' ? 'font-semibold text-blue-600' : 'text-gray-700'}`}
                        onClick={() => { setSelectedCategory('Customer'); setShowCategoryDropdown(false); }}
                        role="option"
                        aria-selected={selectedCategory === 'Customer'}
                      >
                        Customers
                      </li>
                      <li
                        className={`px-4 py-2 cursor-pointer rounded-lg transition-all hover:bg-blue-50 ${selectedCategory === 'Supplier' ? 'font-semibold text-blue-600' : 'text-gray-700'}`}
                        onClick={() => { setSelectedCategory('Supplier'); setShowCategoryDropdown(false); }}
                        role="option"
                        aria-selected={selectedCategory === 'Supplier'}
                      >
                        Suppliers
                      </li>
                      <li
                        className={`px-4 py-2 cursor-pointer rounded-lg transition-all hover:bg-blue-50 ${selectedCategory === 'Debtors' ? 'font-semibold text-blue-600' : 'text-gray-700'}`}
                        onClick={() => { setSelectedCategory('Debtors'); setShowCategoryDropdown(false); }}
                        role="option"
                        aria-selected={selectedCategory === 'Debtors'}
                      >
                        Debtors
                      </li>
                      <li
                        className={`px-4 py-2 cursor-pointer rounded-lg transition-all hover:bg-blue-50 ${selectedCategory === 'Creditors' ? 'font-semibold text-blue-600' : 'text-gray-700'}`}
                        onClick={() => { setSelectedCategory('Creditors'); setShowCategoryDropdown(false); }}
                        role="option"
                        aria-selected={selectedCategory === 'Creditors'}
                      >
                        Creditors
                      </li>
                    </ul>,
