...
function FeatureCard({ icon: Icon, title, description, delay = 0 }: { icon: any; title: string; description: string; delay?: number }) {
  return (
    <div 
      className="group bg-white/80 backdrop-blur-sm rounded-2xl p-6 sm:p-8 shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 hover:border-indigo-200 transform hover:-translate-y-2"
      style={{ animationDelay: `${delay}ms` }}
    >
      <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300">
        <Icon className="w-6 h-6 sm:w-7 sm:h-7 text-white" />
      </div>
      <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-2 sm:mb-3 group-hover:text-indigo-600 transition-colors">{title}</h3>
...
function TestimonialCard({ name, business, quote, rating, avatar }: { name: string; business: string; quote: string; rating: number; avatar?: string }) {
  ...
  return (
    <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 sm:p-8 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
      <div className="flex mb-3 sm:mb-4">
        {[...Array(5)].map((_, i) => (
          <Star key={i} className={`w-4 h-4 sm:w-5 sm:h-5 ${i < rating ? 'text-yellow-400 fill-current' : 'text-gray-200'}`} />
        ))}
      </div>
      <p className="text-sm sm:text-base text-gray-700 mb-4 sm:mb-6 italic leading-relaxed">"{quote}"</p>
      <div className="flex items-center justify-center">
        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-indigo-400 to-purple-500 rounded-full flex items-center justify-center text-white font-bold text-base sm:text-lg mr-3 sm:mr-4">
          <span>{initials}</span>
        </div>
        <div>
...
function Navigation({ onGetStarted, onSignIn }: { onGetStarted: () => void; onSignIn: () => void }) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-40 bg-gradient-to-r from-white/80 via-indigo-50/80 to-white/80 shadow-lg shadow-indigo-100 backdrop-blur-md border-b border-gray-200/50 w-full rounded-b-2xl relative before:absolute before:inset-0 before:bg-white/40 before:backdrop-blur-[8px] before:rounded-b-2xl before:pointer-events-none">
      <div className="flex items-center justify-between h-16 sm:h-20 w-full px-4 sm:px-8 lg:px-16 relative z-10">
        {/* Logo - always left */}
        <div className="flex items-center flex-shrink-0 hover:scale-105 transition-transform duration-200 drop-shadow-lg">
          <img src="/devease_logo.svg" alt="Logo" className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl object-contain bg-white shadow-indigo-100 shadow-lg" />
          <h1 className="text-lg sm:text-2xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent ml-2 sm:ml-3">Devease Digital</h1>
        </div>

        {/* Nav links + actions - always right */}
        <div className="hidden md:flex items-center space-x-4 lg:space-x-6">
          <a href="#features" className="group text-gray-600 hover:text-indigo-600 transition-colors font-medium relative hover:scale-105 duration-200 text-sm lg:text-base">
            Features
          ...
          <a href="#testimonials" className="group text-gray-600 hover:text-indigo-600 transition-colors font-medium relative hover:scale-105 duration-200 text-sm lg:text-base">
            Reviews
            <span className="absolute left-0 -bottom-1 w-full h-0.5 bg-indigo-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left rounded-full" />
          </a>
          <a href="#pricing" className="group text-gray-600 hover:text-indigo-600 transition-colors font-medium relative hover:scale-105 duration-200 text-sm lg:text-base">
            Pricing
            <span className="absolute left-0 -bottom-1 w-full h-0.5 bg-indigo-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left rounded-full" />
          </a>
          <a href="#support" className="group text-gray-600 hover:text-indigo-600 transition-colors font-medium relative hover:scale-105 duration-200 text-sm lg:text-base">
            Support
            <span className="absolute left-0 -bottom-1 w-full h-0.5 bg-indigo-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left rounded-full" />
          </a>
          <div className="border-l border-gray-300 h-6 lg:h-8 mx-2 lg:mx-4" />
          <button
            onClick={onSignIn}
            className="text-gray-700 font-semibold border border-indigo-200 rounded-lg px-3 lg:px-5 py-2 lg:py-2.5 bg-white shadow-md hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 hover:text-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-300 transition-all duration-200 hover:scale-105 text-sm lg:text-base"
          >
            Sign In
          </button>
          <button
            onClick={onGetStarted}
            className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-4 lg:px-6 py-2 lg:py-2.5 rounded-xl ring-1 ring-indigo-100 hover:ring-4 hover:ring-indigo-300/40 hover:from-indigo-700 hover:to-purple-700 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 text-sm lg:text-base"
          >
            Get Started
          </button>
        </div>
        ...
        <button
          ...
          className="md:hidden text-gray-600 hover:text-indigo-600 ml-auto p-2"
        >
      ...
      {isOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-xl border-t border-gray-200/50 rounded-b-2xl shadow-xl">
          <div className="px-4 pt-4 pb-6 space-y-3">
            <a href="#features" className="block px-4 py-3 text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all duration-200 font-medium">Features</a>
            <a href="#testimonials" className="block px-4 py-3 text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all duration-200 font-medium">Reviews</a>
            <a href="#pricing" className="block px-4 py-3 text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all duration-200 font-medium">Pricing</a>
            <a href="#support" className="block px-4 py-3 text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all duration-200 font-medium">Support</a>
            <div className="border-t border-gray-200 my-3"></div>
            <button onClick={onSignIn} className="block w-full text-left px-4 py-3 text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all duration-200 font-medium">Sign In</button>
            <button onClick={onGetStarted} className="block w-full text-left px-4 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-semibold hover:from-indigo-700 hover:to-purple-700 transition-all duration-200">Get Started</button>
          </div>
        </div>
      )}
    </nav>
...
function FeaturesTypewriterHeading() {
  ...
  return (
    <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-2 min-h-[3.5em] px-4 sm:px-0">
      <span>
        {line1}
        {isTypingFirstLine && (
          <span className="inline-block w-1 sm:w-2 align-middle">
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 relative overflow-hidden">
      ...
      >
        ...
        <div className="absolute -bottom-16 sm:-bottom-32 -right-16 sm:-right-32 w-48 h-48 sm:w-96 sm:h-96 bg-gradient-to-tr from-blue-400/30 to-cyan-600/20 rounded-full blur-3xl animate-pulse delay-1000 z-0"></div>
        <div className="container mx-auto px-4 sm:px-6 lg:max-w-none lg:px-8 xl:px-24 relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-8 sm:gap-12 h-full w-full">
            ...
            <div className="w-full lg:w-[40%] flex items-center justify-center relative mt-8 lg:mt-0">
              {/* Floating business icons illustration - hidden on mobile */}
              ...
              {/* Auth Form */}
              <div className="relative w-full max-w-sm sm:max-w-lg mx-auto h-auto max-h-[90vh] flex flex-col justify-center p-4 sm:p-6 rounded-3xl shadow-2xl bg-white/90 backdrop-blur-xl border border-white/20 z-10 my-8 sm:my-12 box-border before:content-[''] before:absolute before:inset-0 before:bg-gradient-to-br before:from-indigo-100/60 before:to-purple-100/40 before:rounded-3xl before:blur-xl before:-z-10">
                {/* Auth Toggle */}
                <div className="flex bg-gray-100 rounded-2xl p-1 mb-4 py-1">
                  <button
                    ...
                    className={`flex-1 py-2.5 sm:py-3 px-4 sm:px-6 rounded-xl text-xs sm:text-sm font-bold transition-all duration-300 ${
                        ...
                        <>
                          <div>
                            ...
                            {formErrors.businessName && (
                              <p className="text-red-500 text-xs sm:text-sm mt-1 font-medium">{formErrors.businessName}</p>
                            )}
                          </div>
                          <div>
                            <label className="block text-sm sm:text-base font-bold text-gray-700 mb-1">
                            ...
                            />
                            {formErrors.phone && (
                              <p className="text-red-500 text-xs sm:text-sm mt-1 font-medium">{formErrors.phone}</p>
                            )}
                          </div>
                        </>
                      )}
      ...
      {/* Features Section */}
      <section id="features" className="py-16 sm:py-20 lg:py-24 bg-white/50 backdrop-blur-sm relative min-h-screen">
        <div className="w-full">
          <div className="text-center mb-12 sm:mb-16 animate-fadeInUp px-4 sm:px-6">
            <div className="inline-flex items-center bg-indigo-100 rounded-full px-4 sm:px-6 py-2 mb-4 sm:mb-6">
              <Zap className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-600 mr-2" />
              <span className="text-xs sm:text-sm font-bold text-indigo-600">POWERFUL FEATURES</span>
            </div>
            <FeaturesTypewriterHeading />
            <p className="text-base sm:text-lg lg:text-xl text-gray-600 max-w-3xl mx-auto mt-2 px-4 sm:px-0">
              Powerful features designed specifically for modern businesses to streamline operations and boost growth
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-12 px-4 sm:px-6 lg:px-8 xl:px-10">
      ...
      {/* Stats Section */}
      <section className="py-16 sm:py-20 lg:py-24 bg-gradient-to-br from-indigo-600 via-purple-600 to-blue-600 relative overflow-hidden">
        {/* Animated background shapes */}
        <div className="absolute inset-0 bg-black/20 z-0"></div>
        <div className="absolute -top-16 sm:-top-32 -left-16 sm:-left-32 w-48 h-48 sm:w-96 sm:h-96 bg-gradient-to-br from-indigo-400/30 to-purple-600/20 rounded-full blur-3xl animate-pulse z-0"></div>
        <div className="absolute -bottom-16 sm:-bottom-32 -right-16 sm:-right-32 w-48 h-48 sm:w-96 sm:h-96 bg-gradient-to-tr from-blue-400/30 to-cyan-600/20 rounded-full blur-3xl animate-pulse delay-1000 z-0"></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-8 sm:mb-10 animate-fadeInUp px-4 sm:px-0">
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-3">Trusted by Hundreds</h2>
            <p className="text-base sm:text-lg text-indigo-100 mb-2 animate-fadeInUp delay-200">Join 1,000+ businesses who trust Devease Digital for their daily operations</p>
            <p className="text-sm sm:text-base text-white/80 animate-fadeInUp delay-400">Your business is in safe hands with our secure, reliable, and scalable platform.</p>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 lg:gap-12 text-center text-white px-4 sm:px-0">
            {[{
              value: '1K+',
              label: 'Active Users'
            }, {
              ...
              <div
                key={stat.label}
                className="bg-white/20 backdrop-blur-lg rounded-2xl p-6 sm:p-8 lg:p-10 border border-white/30 shadow-xl transition-transform duration-300 hover:scale-105 hover:shadow-2xl animate-fadeInUp"
                style={{ animationDelay: `${idx * 120 + 400}ms`, animationFillMode: 'both' }}
              >
                <div className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-extrabold mb-2 drop-shadow-lg">{stat.value}</div>
                <div className="text-white/90 font-semibold text-sm sm:text-base lg:text-lg tracking-wide">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="testimonials" className="py-16 sm:py-20 lg:py-24 bg-gray-50/50 backdrop-blur-sm relative overflow-hidden">
        {/* Animated background shapes */}
        <div className="absolute -top-16 sm:-top-32 -right-16 sm:-right-32 w-48 h-48 sm:w-96 sm:h-96 bg-gradient-to-br from-emerald-400/30 to-blue-600/20 rounded-full blur-3xl animate-pulse z-0"></div>
        <div className="absolute -bottom-16 sm:-bottom-32 -left-16 sm:-left-32 w-48 h-48 sm:w-96 sm:h-96 bg-gradient-to-tr from-blue-400/30 to-cyan-600/20 rounded-full blur-3xl animate-pulse delay-1000 z-0"></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-8 sm:mb-10 animate-fadeInUp px-4 sm:px-0">
            <div className="inline-flex items-center bg-emerald-100 rounded-full px-4 sm:px-6 py-2 mb-4 sm:mb-6">
              <Star className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-600 mr-2" />
              <span className="text-xs sm:text-sm font-bold text-emerald-600">CUSTOMER REVIEWS</span>
            </div>
            <h2 className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-bold text-gray-900 mb-3 px-4 sm:px-0">
              What Our Customers
              <span className="block bg-gradient-to-r from-emerald-600 to-blue-600 bg-clip-text text-transparent pb-2 leading-[1.15]">
                Are Saying
              </span>
            </h2>
            <p className="text-base sm:text-lg text-emerald-700 mb-2 animate-fadeInUp delay-200">Real feedback from real businesses using Devease Digital</p>
            <p className="text-sm sm:text-base text-gray-600 animate-fadeInUp delay-400">We're proud to help thousands of businesses grow and succeed every day.</p>
          </div>
      ...
      {/* CTA Section */}
      <section className="py-16 sm:py-20 lg:py-24 bg-gradient-to-br from-gray-50 to-indigo-50 relative overflow-hidden">
        {/* Animated background shapes */}
        <div className="absolute -top-16 sm:-top-32 -left-16 sm:-left-32 w-48 h-48 sm:w-96 sm:h-96 bg-gradient-to-br from-indigo-400/30 to-purple-600/20 rounded-full blur-3xl animate-pulse z-0"></div>
        <div className="absolute -bottom-16 sm:-bottom-32 -right-16 sm:-right-32 w-48 h-48 sm:w-96 sm:h-96 bg-gradient-to-tr from-blue-400/30 to-cyan-600/20 rounded-full blur-3xl animate-pulse delay-1000 z-0"></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <div className="max-w-4xl mx-auto animate-fadeInUp">
            <h2 className="text-2xl sm:text-3xl lg:text-4xl xl:text-6xl font-bold text-gray-900 mb-4 px-4 sm:px-0">
              Ready to Transform
              <span className="block bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent pb-2 leading-[1.15]">
                Your Business?
              </span>
            </h2>
            <p className="text-base sm:text-lg lg:text-xl text-indigo-700 mb-4 sm:mb-6 animate-fadeInUp delay-200 px-4 sm:px-0">
              Join thousands of businesses already using Devease Digital to streamline operations and boost growth
            </p>
            <p className="text-sm sm:text-base text-gray-600 mb-8 sm:mb-10 animate-fadeInUp delay-400 px-4 sm:px-0">
              Get started today and see the difference for yourself.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center items-center max-w-md mx-auto px-4 sm:px-0">
              <button 
                onClick={() => {
                  setIsLogin(false);
                  setCurrentStep(1);
                ...
                className="w-full sm:w-auto bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-6 sm:px-10 py-3 sm:py-4 rounded-2xl hover:from-indigo-700 hover:to-purple-700 transition-all duration-300 font-bold shadow-2xl hover:shadow-3xl transform hover:-translate-y-1 flex items-center justify-center animate-fadeInUp delay-600 text-sm sm:text-base"
              >
                Start Free Trial
                <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 ml-2" />
              </button>
              <button 
                ...
                className="w-full sm:w-auto border-2 border-gray-300 text-gray-700 px-6 sm:px-10 py-3 sm:py-4 rounded-2xl hover:bg-gray-50 transition-all duration-300 font-bold hover:border-gray-400 animate-fadeInUp delay-800 text-sm sm:text-base"
              >
                Sign In
              </button>
            </div>
            ...
            <div className="col-span-1 sm:col-span-2">
              <div className="flex items-center mb-3">
                <div className="w-6 h-6 sm:w-8 sm:h-8 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-lg flex items-center justify-center">
                  <img src="/devease_logo.svg" alt="Logo" className="w-6 h-6 sm:w-8 sm:h-8 rounded-lg object-contain bg-white" />
                </div>
                <span className="text-lg sm:text-xl font-bold text-white ml-2">Devease Digital</span>
              </div>
              ...
              <div className="flex space-x-2 mt-2">
                <a href="#" className="w-6 h-6 sm:w-8 sm:h-8 bg-gray-800 rounded-lg flex items-center justify-center hover:bg-indigo-600 transition-colors cursor-pointer" aria-label="Website">
                  <Globe className="w-3 h-3 sm:w-4 sm:h-4" />
                </a>
                <a href="#" className="w-6 h-6 sm:w-8 sm:h-8 bg-gray-800 rounded-lg flex items-center justify-center hover:bg-indigo-600 transition-colors cursor-pointer" aria-label="Facebook">
                  <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M22.675 0h-21.35C.595 0 0 .592 0 1.326v21.348C0 23.408.595 24 1.325 24h11.495v-9.294H9.692v-3.622h3.128V8.413c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.797.143v3.24l-1.918.001c-1.504 0-1.797.715-1.797 1.763v2.313h3.587l-.467 3.622h-3.12V24h6.116C23.406 24 24 23.408 24 22.674V1.326C24 .592 23.406 0 22.675 0"/></svg>
                </a>
                <a href="#" className="w-6 h-6 sm:w-8 sm:h-8 bg-gray-800 rounded-lg flex items-center justify-center hover:bg-indigo-600 transition-colors cursor-pointer" aria-label="Twitter">
                  <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 4.557a9.93 9.93 0 0 1-2.828.775 4.932 4.932 0 0 0 2.165-2.724c-.951.564-2.005.974-3.127 1.195a4.92 4.92 0 0 0-8.384 4.482C7.691 8.095 4.066 6.13 1.64 3.161c-.542.929-.856 2.01-.857 3.17 0 2.188 1.115 4.117 2.823 5.254a4.904 4.904 0 0 1-2.229-.616c-.054 2.281 1.581 4.415 3.949 4.89a4.936 4.936 0 0 1-2.224.084c.627 1.956 2.444 3.377 4.6 3.417A9.867 9.867 0 0 1 0 21.543a13.94 13.94 0 0 0 7.548 2.209c9.057 0 14.009-7.496 14.009-13.986 0-.213-.005-.425-.014-.636A9.936 9.936 0 0 0 24 4.557z"/></svg>
                </a>
              </div>
            </div>
            <div>
              <h3 className="font-bold text-white mb-2 text-xs sm:text-sm">Product</h3>
              <ul className="space-y-1">
                <li><a href="#features" className="hover:text-white transition-colors text-xs sm:text-sm">Features</a></li>
                <li><a href="#pricing" className="hover:text-white transition-colors text-xs sm:text-sm">Pricing</a></li>
                <li><a href="#" className="hover:text-white transition-colors text-xs sm:text-sm">API</a></li>
              </ul>
            </div>
            <div>
              <h3 className="font-bold text-white mb-2 text-xs sm:text-sm">Support</h3>
              <ul className="space-y-1">
                <li><a href="#" className="hover:text-white transition-colors text-xs sm:text-sm">Help Center</a></li>
                <li><a href="#" className="hover:text-white transition-colors text-xs sm:text-sm">Contact Us</a></li>
                <li><a href="#" className="hover:text-white transition-colors text-xs sm:text-sm">Status</a></li>
              </ul>
            </div>
            <div>
              <h3 className="font-bold text-white mb-2 text-xs sm:text-sm">Legal</h3>
              <ul className="space-y-1">
                <li><a href="#" className="hover:text-white transition-colors text-xs sm:text-sm">Privacy</a></li>
                <li><a href="#" className="hover:text-white transition-colors text-xs sm:text-sm">Terms</a></li>
                <li><a href="#" className="hover:text-white transition-colors text-xs sm:text-sm">Cookies</a></li>
              </ul>
            </div>
          </div>
          <div className="border-t border-gray-800 pt-4 flex flex-col sm:flex-row justify-between items-center">
            <p className="text-xs mb-2 sm:mb-0">© 2025 Devease Digital. All rights reserved.</p>
