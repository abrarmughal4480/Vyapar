...
export default function BackupRestorePage() {
  ...
  const checkAuth = useCallback(() => {
    try {
      ...
      // Create businessId if not exists
      if (!parsedUser.businessId) {
        parsedUser.businessId = parsedUser.id || `business-${Date.now()}`;
  ...
  }, [checkAuth, router, getMockBackupResponse])
  ...
  const fetchBackupHistory = useCallback(async () => {
    try {
      ...
      try {
        const tableStatusResponse = await fetch('/api/backup/table-status', {
          headers: auth.headers
        })
        
        if (tableStatusResponse.ok) {
          const tableStatus = await tableStatusResponse.json()
          setBackupTableExists(tableStatus.tableExists || false)
        ...
        console.warn('Could not check table status:', tableError)
        setBackupTableExists(false)
      }

      const result = await makeApiCall(`/backup/history/${auth.user.businessId}`)
  ...
  const createBackupTable = useCallback(async () => {
    try {
      ...
      const response = await fetch('/api/backup/create-table', {
        method: 'POST',
        headers: auth.headers
      })
      ...
      if (response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to create backup table')
      }
    } catch (error: any) {
  ...
  const handleBackup = useCallback(async () => {
    ...
    if (!backupTableExists) {
      const shouldCreate = confirm(
        'Backup table does not exist. Would you like to create it first?'
      )
    ...
    try {
      ...
      const result = await makeApiCall(`/backup/create/${auth.user.businessId}`, {
        method: 'POST',
        body: JSON.stringify({
  ...
  }, [checkAuth, selectedBackupType, makeApiCall, downloadBackupFile, backupTableExists, createBackupTable, fetchBackupHistory])
  ...
  const confirmRestore = useCallback(async () => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const result = await makeApiCall(`/backup/restore/${auth.user.businessId}`, {
          method: 'POST',
  ...
  }, [selectedFileForRestore, checkAuth, makeApiCall, fetchBackupHistory])
  ...
  const handleDeleteBackup = useCallback(async (backupId: number, backupName: string) => {
    ...
    try {
      const result = await makeApiCall(`/backup/${auth.user.businessId}/${backupId}`, {
        method: 'DELETE'
      })
  ...
  const handleDownloadBackup = useCallback(async (backupId: number, backupType: string) => {
    ...
    try {
      const result = await makeApiCall(`/backup/${auth.user.businessId}/${backupId}/download`)
  ...
  }, [checkAuth, makeApiCall, downloadBackupFile])
