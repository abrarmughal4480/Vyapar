...
export default function CreditNotePage() {
  ...
  const checkAuth = useCallback(() => {
    ...
    try {
      ...
      console.log('Credit Notes: Auth check successful for real user:', parsedUser.name || parsedUser.email);
  ...
  const simulateLocalApiCall = useCallback(async (endpoint: string, options: RequestInit = {}): Promise<any> => {
    ...
    const localKey = `vyparr_credit_notes_${businessId}`;

    console.log('Using local storage for endpoint:', endpoint);

    if (endpoint.includes('/credit-note')) {
      if (options.method === 'POST') {
        ...
        localStorage.setItem(localKey, JSON.stringify(existingNotes));
        
        console.log('Credit note saved to localStorage:', newNote.id);
        
        return { success: true, data: newNote };
      }
      
      if (endpoint.includes('generate-number')) {
        const existingNotes = JSON.parse(localStorage.getItem(localKey) || '[]');
        const nextNumber = existingNotes.length + 1;
        return {
          success: true,
          data: { noteNumber: `CN-${Date.now()}-${String(nextNumber).padStart(3, '0')}` }
        };
      }
      
      const savedNotes = JSON.parse(localStorage.getItem(localKey) || '[]');
      console.log(`Loaded ${savedNotes.length} credit notes from localStorage`);
  ...
  const fetchCreditNotes = useCallback(async () => {
    try {
      ...
      const result = await makeApiCall(`/credit-note/${auth.user.businessId}`);
      
      if (result.success && Array.isArray(result.data)) {
        setCreditNotes(result.data);
        console.log(`Loaded ${result.data.length} credit notes successfully`);
      } else {
