...
import { reportsApi, BusinessSummary, MonthlyData, TopProduct } from '@/lib/api/reports'
...
export default function ReportsPage() {
  ...
  // Replace with actual business ID from auth context
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true)
        const [summary, income, products] = await Promise.all([
          reportsApi.getBusinessSummary(businessId),
          reportsApi.getMonthlyIncome(businessId, currentYear),
          reportsApi.getTopProducts(businessId, 5)
        ])
                ...
                {monthlyIncome.length === 0 ? (
                  ...
                  <div className="space-y-4 h-full overflow-y-auto">
                    {monthlyIncome.slice(0, 8).map((data) => {
                      const maxValue = Math.max(...monthlyIncome.map(d => d.totalIncome))
                      const percentage = maxValue > 0 ? (data.totalIncome / maxValue) * 100 : 0
                      return (
                        <div key={data.month} className="flex items-center space-x-4 group">
                          <div className="w-16 text-sm font-semibold text-gray-600">
                            {new Date(0, data.month - 1).toLocaleString('default', { month: 'short' })}
                          </div>
